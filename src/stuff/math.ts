...
// Point type for geometric calculations
...
export default {
    ...
    // p1 = point, (p2, p3) = line
    point2line(p1: Point, p2: Point, p3: Point): number {
    ...
    point2seg(p1: Point, p2: Point, p3: Point): number {
    ...
    // p1 = point, (p2, p3) = ray
    point2ray(p1: Point, p2: Point, p3: Point): number {
    ...
    tri(p1: Point, p2: Point, p3: Point): Triangle {
    ...
    /* Area of triangle:
        ...
        p2  _  p3
    */
    area(p1: Point, p2: Point, p3: Point): number {
	     return (
             p1[0] * (p2[1] - p3[1]) +
             p2[0] * (p3[1] - p1[1]) +
             p3[0] * (p1[1] - p2[1])
    ...
    // Triangle height
    tri_h(area: number, base: number): number {
	    return area / base
    },

    // Dot product of (p2, p3) and (p2, p1)
    dot_prod(p1: Point, p2: Point, p3: Point): number {
        let v1: Point = [p3[0] - p2[0], p3[1] - p2[1]]
        let v2: Point = [p1[0] - p2[0], p1[1] - p2[1]]
    ...
    log_mid(r: [number, number], h: number): number {
    ...
    re_range(r1: [number, number], hi2: number, mid: number): number {
