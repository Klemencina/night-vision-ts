
// Dataset proxy between vuejs & WebWorker

import { now } from './script_utils'
import type se from './script_engine'

interface DatasetDescriptor {
    type: string
    id: string
    data?: any[]
}

interface DatasetOp {
    id: string
    type: 'set' | 'del' | 'mrg'
    data?: any
    exec?: boolean
}

interface DataCube {
    ww: WebWorkerInterface
    data: { chart: { data: any[] } }
    dss: Record<string, Dataset>
    del(path: string): void
}

interface WebWorkerInterface {
    just(type: string, data: any): void
    exec(type: string, id: string): Promise<any>
}

export default class Dataset {
    type: string
    id: string
    dc: DataCube

    constructor(dc: DataCube, desc: DatasetDescriptor) {
        // TODO: dataset url arrow fn tells WW
        // to load the ds directly from web

        this.type = desc.type
        this.id = desc.id
        this.dc = dc

        // Send the data to WW
        if (desc.data) {
            this.dc.ww.just('upload-data', {
                [this.id]: desc
            })
            // Remove the data from the descriptor
            delete desc.data
        }

        let proto = Object.getPrototypeOf(this)
        Object.setPrototypeOf(desc, proto)
        Object.defineProperty(desc, 'dc', {
            get() { return dc }
        })
    }

    // Watch for the changes of descriptors
    static watcher(n: DatasetDescriptor[], p: DatasetDescriptor[], self: DataCube): void {
        let nids = n.map(x => x.id)
        let pids = p.map(x => x.id)

        for (var id of nids) {
            if (!pids.includes(id)) {
                let ds = n.filter(x => x.id === id)[0]
                if (ds) {
                    self.dss[id] = new Dataset(self, ds)
                }
            }
        }

        for (var id of pids) {
            if (!nids.includes(id) && self.dss[id]) {
                self.dss[id].remove()
            }
        }
    }

    // Make an object for data transfer
    static make_tx(dc: DataCube, types: { type: string }[]): Record<string, any> {
        let main = dc.data.chart.data
        let base: Record<string, any> = {}
        if (types.find(x => x.type === 'OHLCV')) {
            base = { ohlcv: main }
        }

        return base
    }

    // Set data (overwrite the whole dataset)
    set(data: any[], exec: boolean = true): void {
        this.dc.ww.just('dataset-op', {
            id: this.id,
            type: 'set',
            data: data,
            exec: exec
        } as DatasetOp)
    }

    // Update with new data (array of data points)
    update(arr: any[]): void {
        this.dc.ww.just('update-data', {
            [this.id]: arr
        })
    }

    // Send WW a chunk to merge. The merge algo
    // here is simpler than in DC. It just adds
    // data at the beginning or/and the end of ds
    merge(data: any[], exec: boolean = true): void {
        this.dc.ww.just('dataset-op', {
            id: this.id,
            type: 'mrg',
            data: data,
            exec: exec
        } as DatasetOp)
    }

    // Remove the ds from WW
    remove(exec: boolean = true): void {
        this.dc.del(`datasets.${this.id}`)
        this.dc.ww.just('dataset-op', {
            id: this.id,
            type: 'del',
            exec: exec
        } as DatasetOp)
        delete this.dc.dss[this.id]
    }

    // Fetch data from WW
    async data(): Promise<any[] | undefined> {
        let ds = await this.dc.ww.exec(
            'get-dataset',
            this.id
        )
        if (!ds) return undefined
        return ds.data
    }
}

// Dataset reciever (created on WW)

export class DatasetWW {
    last_upd: number
    id: string
    data: any[]
    type?: string

    constructor(id: string, data: any[] | DatasetDescriptor) {
        this.last_upd = now()
        this.id = id
        if (Array.isArray(data)) {
            // Regular array
            this.data = data
            if (id === 'ohlcv') this.type = 'OHLCV'
        } else {
            // Dataset descriptor
            this.data = data.data || []
            this.type = data.type
        }
    }

    // Update from 'update-data' event
    // TODO: ds size limit (in MB / data points)
    static update_all(se: typeof import('./script_engine').default, data: Record<string, any[]>): void {
        for (var k in data) {
            if (k === 'ohlcv') continue
            let id = k.split('.')[1] || k
            if (!se.data[id]) continue
            let arr = se.data[id].data
            let iN = arr.length - 1
            let last = arr[iN]

            for (var dp of data[k]) {
                if (!last || dp[0] > last[0]) {
                    arr.push(dp)
                }
            }
            se.data[id].last_upd = now()
        }
    }

    merge(data: any[]): void {
        let len = this.data.length
        if (!len) {
            this.data = data
            return
        }
        let t0 = this.data[0][0]
        let tN = this.data[len - 1][0]
        let l = data.filter(x => x[0] < t0)
        let r = data.filter(x => x[0] > tN)
        this.data = l.concat(this.data, r)
    }

    // On dataset operation
    op(se: typeof import('./script_engine').default, op: DatasetOp): void {
        this.last_upd = now()
        switch(op.type) {
            case 'set':
                this.data = op.data as any[]
                se.recalc_size()
                break
            case 'del':
                delete se.data[this.id]
                se.recalc_size()
                break
            case 'mrg':
                this.merge(op.data as any[])
                se.recalc_size()
                break
        }
    }
}
