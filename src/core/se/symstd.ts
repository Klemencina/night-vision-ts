...
export default {
            ...
            if (m) {
                ...
                switch(m[1]){
                    case 'tr':
                        ...
                        se.std_plus[fn] = function(this: any, fixnan: boolean = false, _id?: string) {
                            return this.tr(fixnan, _id, tf)
                        }
                        break
                    case 'atr':
                        this.deps(['high', 'low', 'close'], m[2])
                        se.std_plus[fn] = function(this: any, len: number, _id?: string) {
                            return this.atr(len, _id, tf)
                        }
                        break
                    case 'kc':
                        this.deps(['high', 'low', 'close'], m[2])
                        se.std_plus[fn] = function(this: any, src: any, len: number, mult: number,
                            use_tr: boolean = true, _id?: string) {
                    ...
                    case 'dmi':
                        this.deps(['high', 'low', 'close'], m[2])
                        se.std_plus[fn] = function(this: any, len: number, smooth: number, _id?: string) {
                            return this.dmi(len, smooth, _id, tf)
                        }
                        break
                    case 'sar':
                        this.deps(['high', 'low', 'close'], m[2])
                        se.std_plus[fn] = function(this: any, start: number, inc: number, max: number, _id?: string) {
                            return this.sar(start, inc, max, _id, tf)
                        }
                        break
                    case 'supertrend':
                        this.deps(['high', 'low', 'close'], m[2])
                        se.std_plus[fn] = function(this: any, factor: number, atrlen: number, _id?: string) {
    ...
    deps(types: string[], tf: string): void {
