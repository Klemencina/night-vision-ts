...
const BUF_INC = 5
let tf_cache: Record<string, number> = {}

export function f_args(src: string): string[] {
    FDEFS.lastIndex = 0
...
export function f_body(src: string): string {
    return src.slice(
        src.indexOf("{") + 1,
        src.lastIndexOf("}")
    )
...
export function wrap_idxs(src: string, pre: string = ''): string {
...
export function make_module_lib(mod: Record<string, any>): Record<string, Function> {
...
export function get_raw_src(f: Function | string): string {
...
export function tf_from_pair(num: string, pf: string): number {
    var mult = 1
    switch (pf) {
...
export function tf_from_str(str: string | number): number | undefined {
...
export function get_fn_id(pre: string, id: string): string {
    return pre + '-' + id.split('<-').pop()!
}

// Apply filter for all new overlays
export function ovf(obj: Record<string, any>, f: Function): Record<string, any> {
    var nw: Record<string, any> = {}
    for (var id in obj) {
        nw[id] = {}
        for (var k in obj[id]) {
            if (k === 'data') continue
...
export function nextt(data: any[][], t: number, ti: number = 0): number {
...
export function size_of_dss(data: Record<string, any>): number {
    let bytes = 0
    for (var id in data) {
...
export function size_of(object: any): number {
    var list: any[] = [], stack: any[] = [object], bytes = 0
    while (stack.length) {
        var value = stack.pop()
...
export function overrideOverlay(ov1: Record<string, any>, ov2: Record<string, any>): Record<string, any> {
