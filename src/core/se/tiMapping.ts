...
export default class TI {
    ...
    init(params: InitParams, res: any[]): void {
        let {
    ...
    parse(data: any[], mode: string): any[] {
        ...
        if (mode === 'calc') {
            let shift = Utils.indexShift(this.sub, data)
        ...
        if (data.length) {
            try {
                let k1 = Utils.fastNearest(this.sub, data[0][0])[0]
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            let tk = this.sub[k][0]
            let t = data[i][0]
            let index: number | undefined = this.tiMap[t]
    ...
    i2t_mode(i: number, mode: string): number | undefined {
    ...
    t2i(t: number): number | undefined {

        if (!this.sub.length) return undefined

        // Discrete mapping
        let res = this.tiMap[t]
        if (res !== undefined) return res
        ...
        try {
            // Linear Interpolation
            let i = Utils.fastNearest(this.sub, t)
            if (i[0] === null || i[1] === null) return undefined
    ...
    smth2i(smth: number): number | undefined {
        if (smth > MAX_ARR) {
    ...
    smth2t(smth: number): number | undefined {
        if (smth < MAX_ARR) {
    ...
    // Used by tv.goto()
    gt2i(smth: number, ohlcv: any[]): number | undefined {
        if (smth > MAX_ARR) {
            let E = 0.1 // Fixes the arrayslicer bug
            let [i1, i2] = Utils.fastNearest(ohlcv, smth+E)
            if (typeof i1 === 'number') {
                return i1
            } else {
