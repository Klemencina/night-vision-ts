...
export default class Cursor {
    ...
    constructor(meta: CursorMeta) {
        this.meta = meta
    }

    xSync(hub: Hub, layout: Layout, props: Props, update: CursorUpdate): this {
    ...
    xValues(hub: Hub, layout: Layout, props: Props): this {
        ...
        let values: any[][] = []
        let vi: [number, any] | undefined // [value, index]
        for (var pane of hub.panes()) {
            let arr: any[] = []
            for (var i = 0; i < pane.overlays.length; i++) {
                let ov = pane.overlays[i]
                if (!layout.indexBased) {
                    vi = Utils.nearestTs(this.ti!, ov.dataSubset as any) || ([] as any)
                } else {
                    let off = ov.indexOffset ?? 0
                    vi = Utils.nearestTsIb(this.ti!, ov.data as any, off) || ([] as any)
                }
                if (ov.main) {
    ...
    quantizeTime(hub: Hub, layout: Layout, props: Props): void {
